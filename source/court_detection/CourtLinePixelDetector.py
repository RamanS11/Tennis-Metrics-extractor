...
class CourtLinePixelDetector:
    ...
    def __init__(self, debug=False, luminance_min=70, neighborhood_diff=30, court_line_width=2, grad_kernel_size=3,
                 kernel_size=3):
        """
    ...
    def CourtLinePixelDetector(self, frame):
        ...
        self.luminance_img = np.zeros((self.width, self.height))
        self.court_line_pixels = np.zeros((self.width, self.height))

        # obtain luminance channel from input frame.
        self.luminance_img = self.getLuminanceChannel()
    ...
    def detectLinePixels(self):
        ...
        for x in range(0, self.width):
            for y in range(0, self.height):
                ...
                tmp_value = []
                for i in range(1, self.theta):
                    tmp_value.append(self.isLinePixel(x, y, i))

                # Consider evaluated pixel as part of line structure if neighboring pixels follows a line structure.
                if all(v for v in tmp_value):
                    court_line_pixels[x, y] = 255
                else:
                    court_line_pixels[x, y] = 0

        return court_line_pixels

    def isLinePixel(self, x: int, y: int, tmp_theta: int):
        """
        ...
        value = self.luminance_img[x, y]
        # Return False if pixel evaluated is in boundaries of image.
        if (x < tmp_theta or self.width - x <= tmp_theta) or (y < tmp_theta or self.height - y <= tmp_theta):
            return False
        if value < self.lumThr:
            return False

        # Compute vertical and horizontal differences
        dv1 = abs(value - int(self.luminance_img[x, y - tmp_theta]))
        dv2 = abs(value - int(self.luminance_img[x, y + tmp_theta]))
        dh1 = abs(value - int(self.luminance_img[x - tmp_theta, y]))
        dh2 = abs(value - int(self.luminance_img[x + tmp_theta, y]))
